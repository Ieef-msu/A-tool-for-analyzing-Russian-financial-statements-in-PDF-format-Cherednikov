...
CODE_DESCRIPTIONS = {
    ...
    "2410": "Налог на прибыль текущий",
    "2400": "Чистая прибыль (убыток) отчётного периода",
}
...
def extract_codes(md_path: Path) -> dict:
    ...
    for tbl in soup.find_all("table"):
        ...
        # Берём две колонки сразу после колонки с кодами (не последние две!).
        # Это корректно работает и для 2-летних, и для 3-летних форм:
        # [Наименование | Код | 2025 | 2024 | 2023] → cur=2025, prev=2024
        cols_list = list(df.columns)
        code_col_idx = cols_list.index(code_col)
        value_cols = cols_list[code_col_idx + 1:]
        if len(value_cols) < 2:
            continue
        cur_col, prev_col = value_cols[0], value_cols[1]
